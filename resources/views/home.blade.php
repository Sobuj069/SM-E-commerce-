                ...
                <x-card variant="product" :hover3d="true" :image="$product->image" :imageAlt="$product->name" :imageHref="route('product.show', $product->slug)">
                    ...
                    <x-slot:footer>
                        ...
                        <form action="{{ route('cart.add', $product->id) }}" method="POST">
                            ...
                            <x-button variant="secondary" size="sm" type="submit" :fullWidth="true" icon="fa-solid fa-cart-plus">
                                Add To Cart
                            </x-button>
                        </form>
                    </x-slot:footer>
            ...
            @foreach($bestSellers as $product)
                <x-card variant="product" :hover3d="true" :image="$product->image" :imageAlt="$product->name" :imageHref="route('product.show', $product->slug)">
                    ...
                    <x-slot:footer>
                        <div class="flex items-center justify-between">
                            <x-rating :value="$product->rating" :count="$product->reviews_count" size="xs" />
                            <span class="text-lg font-black text-content-primary">${{ number_format($product->effective_price, 2) }}</span>
                        </div>
